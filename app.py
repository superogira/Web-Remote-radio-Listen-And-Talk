...
def handle_audio_chunk(chunk):
    if ptt_process and not ptt_process.stdin.closed:
        try:
...
def stop_ptt():
    ...
    with ptt_lock:
        ...
        socketio.sleep(0.1)
        # 2. กลับมาเริ่ม Thread ฟังเสียง (arecord) ใหม่
        start_radio_listen_thread()
...
def stop_radio_listen_thread():
    ...
    if radio_listen_thread and radio_listen_thread.is_alive():
        ...
        stop_thread.set()
        radio_listen_thread.join() # รอให้ thread ทำงานเสร็จสิ้น
        radio_listen_thread = None
...
if __name__ == '__main__':
    try:
        # ไม่ต้องเริ่ม thread ที่นี่แล้ว จะไปเริ่มเมื่อ client เชื่อมต่อ
