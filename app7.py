...
VOX_THRESHOLD = 500
# [แก้ไข] ปรับให้พร้อมรับเสียงคุณภาพสูงที่ 48kHz
CAPTURE_RATE = "48000"
...
def handle_ptt_start():
    ...
    with ptt_lock:
        ...
        socketio.sleep(0.2) # [แก้ไข] ใช้ sleep เพื่อรอให้ thread หยุด แทน .join()
        ...
        ptt_process = subprocess.Popen(
            ...
            stderr=subprocess.PIPE  # [แก้ไข] เปิดรับ error stream เพื่อ debug
        )
        start_ptt_writer()
...
def stop_ptt():
    ...
    with ptt_lock:
        ...
        stop_ptt_writer()
        if ptt_process:
            # พิมพ์ error (ถ้ามี) จาก aplay
            aplay_stderr = ptt_process.stderr.read()
            if aplay_stderr:
                print(f"aplay stderr: {aplay_stderr.decode('utf-8', errors='ignore')}")
...
def radio_listen():
    # โค้ดส่วนนี้ทำงานได้ดีอยู่แล้ว ไม่ต้องแก้ไข
    ...
    while not stop_thread.is_set():
        ...
        if not audio_chunk:
            print(f"arecord stream ended. stderr: {process.stderr.read().decode('utf-8', errors='ignore')}")
            break
        try:
